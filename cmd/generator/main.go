			...
			for _, item := range mod.Items {
				...
				for _, r := range fixtureName {
					if (r >= 'a' && r <= 'z') || (r >= 'A' && r <= 'Z') || (r >= '0' && r <= '9') || r == '.' || r == '_' {
						cleanName = append(cleanName, r)
					} else {
						cleanName = append(cleanName, '_')
					}
				}
				fixtureName = string(cleanName)
				for strings.Contains(fixtureName, "__") {
				...
				method := &MethodData{
					...
					IsGet:        isGet,
					IsUpload:     strings.Contains(info.Path, "upload"),
					FullAPIName:  item.Name,
					FixtureName:  fixtureName + "_resp.json",
...
{{- if .IsUpload}}
	...
	{{.Name}}FromReader(sid uint64, filename string, reader io.Reader, tok string) (*{{.ResponseType}}, error)
{{- else}}
...
{{- if .IsUpload}}
	res, err := client.{{$.ModuleName}}.{{.Name}}({{if eq .APIType "Shop"}}shopID{{else}}merchantID{{end}}, "fixtures/test.jpg", accessToken)
{{- else}}
	{{if .RequestType -}}
	var req {{.RequestType}}
	{{- end}}
